...
async function browserFetch() {
  ...
  try {
    ...
    if (!html || html.length < 5000) {
      throw new Error(`IM4CAR_BROWSER_SHORT_${html.length}`);
    }
...
global.fetch = async function patchedFetch(input, init) {
  ...
  try {
    ...
    const looksUsable =
      response.ok &&
      text.length >= 5000 &&
      /\/cars\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i.test(text);
