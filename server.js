...
app.use(cors());
const PARTNER_API='https://api.im4car.by/partner/v1';
const ALLOWED_ORIGINS=new Set(['https://red-dragon.by','https://www.red-dragon.by']);
function setProxyCors(req,res){const origin=req.get('origin');if(ALLOWED_ORIGINS.has(origin))res.set('Access-Control-Allow-Origin',origin);res.set('Vary','Origin');res.set('Access-Control-Allow-Methods','GET,OPTIONS');res.set('Access-Control-Allow-Headers','Accept,Content-Type');res.set('Access-Control-Max-Age','86400')}
app.options('/api/im4car/*splat',(req,res)=>{setProxyCors(req,res);res.status(204).end()});
app.get('/api/im4car/*splat',async(req,res)=>{setProxyCors(req,res);try{const key=process.env.IM4CAR_API_KEY;if(!key)return res.status(503).json({ok:false,error:'IM4CAR_API_KEY is not configured'});const suffix='/'+(Array.isArray(req.params.splat)?req.params.splat.join('/'):(req.params.splat||''));if(!/^\/(reference|brands|listings|cars)(\/|$)/.test(suffix))return res.status(404).json({ok:false,error:'Proxy path not allowed'});const u=PARTNER_API+suffix+(req.originalUrl.includes('?')?'?'+req.originalUrl.split('?')[1]:'');const r=await fetch(u,{headers:{Accept:'application/json',Authorization:'Bearer '+key},redirect:'follow',cache:'no-store'});const text=await r.text();res.status(r.status).set('Content-Type',r.headers.get('content-type')||'application/json; charset=utf-8').send(text)}catch(e){res.status(502).json({ok:false,error:'IM4CAR_PROXY_ERROR',message:e.message})}});
