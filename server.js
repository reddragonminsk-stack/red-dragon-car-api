...
async function refresh(){if(refreshing)return cache;refreshing=true;try{const x=await load();const imageResults=await cacheImages(x.cars),cachedCount=imageResults.filter(i=>i.cached).length,failed=imageResults.filter(i=>!i.cached);if(!cachedCount)throw Error('ALL_HOME_IMAGES_CACHE_FAILED');cache={...x,imageCache:{total:imageResults.length,cached:cachedCount,failed:failed.length,updatedAt:new Date().toISOString()}};writeCache(cache);lastError=failed.length?`IMAGE_CACHE_FAILED_${failed.length}`:null;return cache}catch(e){lastError=e.message;throw e}finally{refreshing=false}}
